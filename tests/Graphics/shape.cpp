...
TEST(Geometry2DTests, Triangles)
{
  ...
  // EXPECT_EQ(t_default.get_vertecies_arr(), t_copy.get_vertecies_ref());
  ...
  for (size_t i = 0; i < TEST_ITERATIONS; i++)
  {
    ...
    const double c =
        std::sqrt(a * a + b * b - 2.0 * a * b * cos(angle_A * pi_180th));
    const double angle_B =
        acos((c * c + a * a - b * b) / (2 * a * c)) / pi_180th;
    ...
    EXPECT_NEAR(t_sas.get_perimetr(), a + b + c,
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);
    EXPECT_NEAR(t_sss.get_perimetr(), a + b + c,
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);
    EXPECT_NEAR(t_asa.get_perimetr(), a + b + c,
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);

    const double area = 0.5 * a * b * std::sin(angle_A * pi_180th);
    EXPECT_NEAR(t_sas.get_area(), area,
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);
    EXPECT_NEAR(t_sss.get_area(), area,
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);
    EXPECT_NEAR(t_asa.get_area(), area,
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);

    EXPECT_NEAR(t_sas.get_w(), t_sss.get_w(),
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);
    EXPECT_NEAR(t_sas.get_w(), t_asa.get_w(),
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);
    EXPECT_NEAR(t_sas.get_h(), t_sss.get_h(),
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);
    EXPECT_NEAR(t_sas.get_h(), t_asa.get_h(),
                TEST_FLOATING_POINT_TRIGONOMETRY_TOLERANCE);
  }
}
...
TEST(Geometry2DTests, Circles)
{
  ...
  for (size_t i = 0; i < TEST_ITERATIONS; i++)
  {
    ...
    EXPECT_NEAR(c1.get_area(), M_PI * rand_radius * rand_radius,
                TEST_FLOATING_POINT_TOLERANCE);
    EXPECT_NEAR(c1.get_perimetr(), two_pi * rand_radius,
                TEST_FLOATING_POINT_TOLERANCE);
  }
}
