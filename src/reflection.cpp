...
const char *msce::Type::get_name() const { return this->name_; }
...
bool msce::Type::has_member_named(const char *name) const
{
  for (const auto &m : this->members_)
  {
    if (std::strcmp(m.get_name(), name) == 0) return true;
  }
  return false;
}
...
const msce::MemberInfo &msce::Type::get_member_named(const char *name) const
{
  ...
  for (const auto &m : this->members_)
  {
    if (std::strcmp(m.get_name(), name) == 0) return m;
  }

  logger.log_error("Tried to get member '{}' from '{}', but said object "
                   "doesn't have said member!",
                   name, this->name_);
  throw std::runtime_error("Reflection get_member_named() failed.");
}

uint32_t msce::Type::get_size() const { return this->size_; }

std::vector<std::reference_wrapper<const msce::MemberInfo>>
msce::Type::get_members() const
{
...
uint32_t msce::MemberInfo::get_offset() const { return this->offset_; }
...
const msce::Type &msce::get_reflection_of_type(const std::string &name)
{
  ...
  catch (std::out_of_range &e)
  {
    ...
    l.log_error("Tried to get reflection info of type '{}', but it wasn't "
                "registered with that name. Make sure there's no mismatch "
                "between macros. (Watch out for :: key)",
                name);
    throw std::runtime_error(
        "Reflection function msce::get_reflection_of_type(const std::string&) "
        "defined at '" +
        MSCE_FILE_TRACK_STR() + "' failed for type " + name);
  }
}
...
const msce::Type &msce::get_reflection_of_type(const std::type_info &std_type)
{
  ...
  {
    if (t.get().get_std_type() == std_type) return t;
  }
  throw std::runtime_error(
      "Reflection function msce::get_reflection_of_type(const std::type_info&) "
      "defined at '" +
      MSCE_FILE_TRACK_STR() + "' failed for type " +
      std::string(std_type.name()));
}
