...
#define GET_FIELD_NAME_TYPE_PAIR_HELPER(r, data, x) \
    std::pair(std::string_view(#x), std::reference_wrapper<const std::type_info>(typeid(this->x)))
...
#define MSCE_CEREAL_GENERATE_SERIALIZE_METHODS(...)                                                                                                                  \
    ...
    constexpr std::array<std::pair<std::string_view, std::reference_wrapper<const std::type_info>>, BOOST_PP_VARIADIC_SIZE(__VA_ARGS__)> get_field_name_type_pairs() \
    {                                                                                                                                                                \
        return {BOOST_PP_SEQ_ENUM(BOOST_PP_SEQ_TRANSFORM(GET_FIELD_NAME_TYPE_PAIR_HELPER, _, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__)))};                               \
    }                                                                                                                                                                \
    template <typename T>                                                                                                                                            \
    void set_field(const std::string &name, T value)                                                                                                                 \
    {                                                                                                                                                                \
