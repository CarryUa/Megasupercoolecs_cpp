...
void msce::internal::StackTraceHandler::handle()
{
  ...
  for (int i = 0; i < count - error_trace_start; i++)
  {
    ...
    auto source_info =
        Platform::resolve_shared_object_address_source(reinterpret_cast<void *>(
            reinterpret_cast<uintptr_t>(trace[i + error_trace_start]) -
            1)); // previous instruction.

    if (i != 0)
      l.log_debug("#{}\t\t| '{}' defined at '{}:{}:{}'", i - skipped,
                  Platform::demangle(source_info.so_symbol_name, false),
                  source_info.filename, source_info.line_pos,
                  source_info.col_pos);
    else
      l.log_debug("#0 (err origin)\t| '{}' defined at '{}:{}:{}'",
                  Platform::demangle(source_info.so_symbol_name, false),
                  source_info.filename, source_info.line_pos,
                  source_info.col_pos);
  }
...
#if defined(RELEASE)
  l.log_warning(
      "It seems you are in RELEASE build, which has cryptic stack trace. Run "
      "DEBUG build to see demangled more human-friendly trace.");
#endif
...
void msce::internal::handle_terminate()
{
  ...
  if (std::current_exception())
  {
    ...
    {
      l.log_error("Exception was thrown: {}: '{}'",
                  Platform::demangle(
