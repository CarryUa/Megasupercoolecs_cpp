...
std::string msce::Platform::demangle(const std::string &mangled,
                                     bool log) noexcept
{
  int s = 0;
  ...
  if (!demangled)
  {
    ...
    case -1:
      if (log)
        logger.log_error(
            "Failed to demangle '{}'. Memory allocation failiure occured.",
            mangled);
      break;

    case -2:
      if (log)
        logger.log_error("Failed to demangle '{}'. Invalid mangled input.",
                         mangled);
      break;

    case -3:
      if (log)
        logger.log_error("Failed to demangle '{}'. Invalid argument passed.",
                         mangled);
      break;
...
std::string msce::Platform::get_absolute_executable_path()
{
  static const std::string cached = []()
  {
    ...
    if (len == -1)
    {
      logger.log_error("Failed to get current executable path from system. "
...
msce::Platform::SourceInfo
msce::Platform::resolve_shared_object_address_source(void *address)
{
  SourceInfo res;
  Dl_info info;
  ...
  {
    logger.log_error(
        "Failed to resolve shared object address {} in any linked libraries.",
        address);
    return res;
  }
  ...
  const char *filename =
      dwfl_lineinfo(line, &ip, &line_pos, &col_pos, nullptr, nullptr);
  const char *symbol_name =
      dwfl_module_addrsym(module, ip, &sym, &section_index);
