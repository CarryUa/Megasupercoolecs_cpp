...
static std::vector<std::reference_wrapper<const MemberInfo>>
    current_prototype_fields;
...
{
  if (type == typeid(int)) return PrototypeFieldTypes::INT;
...
MSCE_ABI_EXPORT_ATTRIB void msce_select_prototype(const char *id)
{
  ...
  if (!ptr)
  {
    proto_abi_logger.log_error(
        "Prototype with id '{}' doesn't exist. Ignoring select...", id);
    return;
  }

  current_prototype_fields.clear();
  current_prototype_selection = ptr;
  current_prototype_type =
      &current_prototype_selection->get_type_info_polymorphic();
...
MSCE_ABI_EXPORT_ATTRIB void msce_get_selected_prototype_id(char *out_id,
                                                           int out_size)
{
  if (!out_id || out_size <= 0) return;
  out_id[0] = '\0';

  if (!current_prototype_selection)
  {
    proto_abi_logger.log_error(
        "Tried to get the id, but no prototype was selected!");
    return;
  }

  int buffer_size = std::min(current_prototype_selection->id.length(),
                             static_cast<size_t>(out_size - 1));
  memcpy(out_id, current_prototype_selection->id.c_str(), buffer_size);
  out_id[buffer_size] = '\0';
}

MSCE_ABI_EXPORT_ATTRIB void
msce_get_selected_prototype_fields_count(int *out_count)
{
  if (!current_prototype_selection)
  {
    proto_abi_logger.log_error(
...
MSCE_ABI_EXPORT_ATTRIB void
msce_get_selected_prototype_field_name(int field_index, char *out_field_name,
                                       int out_size)
{
  if (!out_field_name || out_size <= 0) return;
  out_field_name[0] = '\0';
  ...
  {
    proto_abi_logger.log_error(
        "Tried to get field name of field #{}, but there are only {}!",
        field_index + 1, fields_count);
    return;
  }
...
MSCE_ABI_EXPORT_ATTRIB void
msce_get_selected_prototype_field_value(const char *field_name,
                                        char *out_field_value, int out_size,
                                        int *out_type_enum)
{
  if (!out_field_value || out_size <= 0) return;
  out_field_value[0] = '\0';
  ...
  {
    auto type_enum =
        what_type_am_i(current_prototype_type->get_member_named(field_name)
                           .get_type()
                           .get_std_type());
    std::string str_v;
    ...
    {
      str_v = std::to_string(current_prototype_type->get_member_value<int>(
          reinterpret_cast<const void *>(current_prototype_selection),
          field_name));
      break;
    }
    ...
    {
      str_v =
          std::to_string(current_prototype_type->get_member_value<unsigned int>(
              reinterpret_cast<const void *>(current_prototype_selection),
              field_name));
      break;
    }
    ...
    {
      str_v = std::to_string(current_prototype_type->get_member_value<float>(
          reinterpret_cast<const void *>(current_prototype_selection),
          field_name));
      break;
    }
    ...
    {
      str_v = std::to_string(current_prototype_type->get_member_value<double>(
          reinterpret_cast<const void *>(current_prototype_selection),
          field_name));
      break;
    }
    ...
    {
      str_v = current_prototype_type->get_member_value<std::string>(
          reinterpret_cast<const void *>(current_prototype_selection),
          field_name);
      break;
    }
    ...
    std::size_t buffer_size =
        std::min(str_v.length(), static_cast<size_t>(out_size - 1));
...
MSCE_ABI_EXPORT_ATTRIB void
msce_set_selected_prototype_field_value(const char *field_name,
                                        const void *value, int type_enum)
{
  ...
  case PrototypeFieldTypes::INT:
  {
    ...
    current_prototype_type->set_member_value<int>(
        *current_prototype_selection, field_name, std::any_cast<int>(any_v));
    break;
  }
  case PrototypeFieldTypes::UINT:
  {
    any_v = *static_cast<const unsigned int *>(value);
    current_prototype_type->set_member_value<unsigned int>(
        *current_prototype_selection, field_name,
        std::any_cast<unsigned int>(any_v));
    break;
  }
  case PrototypeFieldTypes::ENUM:
  {
    ...
    auto en = EnumManager::instance->create_enum(
        current_prototype_type->get_member_named(field_name)
            .get_type()
            .get_name(),
        int_v);
    current_prototype_type->set_member_value<int>(
        reinterpret_cast<const void *>(current_prototype_selection), field_name,
        *current_prototype_type, int_v);
  ...
  case PrototypeFieldTypes::FLOAT:
  {
    ...
    current_prototype_type->set_member_value<float>(
        *current_prototype_selection, field_name, std::any_cast<float>(any_v));
    break;
  }
  case PrototypeFieldTypes::DOUBLE:
  {
    any_v = *static_cast<const double *>(value);
    current_prototype_type->set_member_value<double>(
        *current_prototype_selection, field_name, std::any_cast<double>(any_v));
    break;
  }
  case PrototypeFieldTypes::STRING:
  {
    any_v = std::string(static_cast<const char *>(value));
    current_prototype_type->set_member_value<std::string>(
        *current_prototype_selection, field_name,
        std::any_cast<std::string>(any_v));
    break;
  }
  }
}

MSCE_ABI_EXPORT_ATTRIB void msce_create_prototype_instance(const char *type,
                                                           const char *new_id,
                                                           bool *success)
{
  if (!success) return;
  ...
  *success = false;
  if (!type || !new_id) return;

  auto ptr =
      PrototypeManager::instance->create_new_prototype_instance(type, new_id);
  if (!ptr) return;
...
MSCE_ABI_EXPORT_ATTRIB void
msce_serialize_prototype(const char *dest_path, const char *id, bool *success)
{
  if (!success) return;
  *success = false;

  if (!dest_path || !id) return;
...
MSCE_ABI_EXPORT_ATTRIB void msce_deserialize_prototype(const char *target_path,
                                                       char *out_id,
                                                       int out_id_size)
{
  if (out_id_size == 0 || !out_id) return;
  out_id[0] = '\0';
  ...
  int buffer_size =
      std::min(proto->id.length(), static_cast<size_t>(out_id_size - 1));
...
MSCE_ABI_EXPORT_ATTRIB void msce_select_enum(const char *enum_name)
{
  ...
  {
    enum_abi_logger.log_warning(
        "Enum with name '{}' does not exist. Ignoring select...", enum_name);
    return;
  }

  current_enum_selection.reset(
      new Enum(EnumManager::instance->create_enum(enum_name, 0)));
}
...
MSCE_ABI_EXPORT_ATTRIB void msce_get_selected_enum_name(char *out_enum_name,
                                                        int out_buffer_size)
{
  if (!out_enum_name || out_buffer_size <= 0) return;
  ...
  int bytes_needed =
      std::min(out_buffer_size - 1, static_cast<int>(name.length()));
  memcpy(out_enum_name, name.c_str(), bytes_needed);
  out_enum_name[bytes_needed] = '\0';
}

MSCE_ABI_EXPORT_ATTRIB void
msce_get_selected_enum_enumerator_name(char *out_enumerator_name,
                                       int out_buffer_size)
{
  if (!out_enumerator_name || out_buffer_size <= 0) return;
...
MSCE_ABI_EXPORT_ATTRIB void
msce_set_selected_enum_enumerator_name(const char *enumerator_name)
{
  if (!enumerator_name) return;
