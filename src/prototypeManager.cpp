...
void msce::PrototypeManager::load_all_prototypes()
{
  ...
  for (auto e : std::filesystem::recursive_directory_iterator(
           Platform::get_absolute_executable_directory_path()))
  {
...
msce::IPrototype *
msce::PrototypeManager::deserialize_prototype(const string &path)
{
  ifstream file(path);
  if (!file)
  {
  ...
  {
    logger.log_error("Couldn't deserialize prototype at '{}': {}", path,
                     e.what());
    return nullptr;
  }
  catch (...)
...
void msce::PrototypeManager::serialize_prototype(const std::string &path,
                                                 const std::string &id)
{
  ...
  {
    logger.log_error("Couldn't serialize '{}' into file at '{}': {}", id, path,
                     e.what());
  }
  catch (...)
  {
...
msce::IPrototype *msce::PrototypeManager::create_new_prototype_instance(
    const std::string &type, const std::string &id) noexcept
{
  if (!registered_factories_ref.is_registered(type))
  {
    logger.log_error("Error: Prototype with type '{}' wasn't ever registered!",
                     type);
    return nullptr;
  }
  if (this->prototypes_.contains(id))
...
bool msce::PrototypeManager::prototype_id_exists(
    const std::string &id) const noexcept
{
  return this->prototypes_.contains(id);
}
bool msce::PrototypeManager::prototype_type_registered(
    const std::string &type) const noexcept
{
  return registered_prototypes_ref.is_registered(type) &&
         registered_prototypes_ref.is_registered(type);
}
bool msce::PrototypeManager::delete_prototype(const std::string &id) noexcept
{
  if (!prototype_id_exists(id)) return false;
