...
template <typename T> constexpr const Type &get_reflection_of_type();
 ...
 */
template <typename T> inline constexpr uint16_t compute_type_traits()
{
  uint16_t flags = 0;
  ...
  // Important to notice here, that I explicitly exclude function and member
  // function pointers from POINTER category. They fall into FUNCTION category,
  // since I use std::is_invocable_v to check for functions.
...
inline Registry<std::string, std::reference_wrapper<const Type>> &
get_g_reflection_types_registry()
{
...
/**
 * @brief Internal function used by reflection. Calculates the offset of member
 * pointer passed in.
 * @tparam TC Type of compound containint member. Dublicate it into Context
 * tparam.
 * @tparam TM Type of member under the pointer.
 * @tparam Context The actual type. You need to pass it it to avoid template
 * deduction.
 */
...
//     //     reinterpret_cast<const char *>(m) - reinterpret_cast<const char
//     *>(c));
// }
} // namespace internal
...
public:
  ...
  /**
   * @brief Sets the value of member represented by this in given object if
   * present.
   * @param object Reference to target object.
  ...
  /**
   * @retval Returns type-casted pointer to the member represented by this in
   * given object if present.
  ...
  /**
   * @retval Returns untyped(void) pointer to the member represented by this in
   * given object if present.
   * @retval nullptr otherwise.
   */
...
private:
  ...
  /**
   * @brief These are flags containing this type's traits. You can find flags at
   * @ref msce::TypeTraitFlags.
   * @details This is not conventional bitmap. Bits 0 trough 2 are most
   * significant. They define mutually-exclusive type "category", like class,
   * numeric, enum, etc.
   * @details Bits 3 trough 5 are category-exclusive specifications. (eg.
   * INGTEGRAL flag is exclusive to NUMERIC category).
   * @details Remaining bits are shader between all categories. (eg. any type
   * can be constant).
   * @note 1) Note that lack of UNSIGNED flag is treated as number being signed.
   * @note 2) Note that lack of BOUNDED flag is treated as array being
   * unbounded.
   * @warning Checking the flags requires special mask(see @ref msce::Type
   * is_*() functions implementation for more info), therefore it is not
   * recomended to use them directly.
   */
...
public:
  constexpr Type(const char *name, uint64_t size, uint16_t trait_flags,
                 std::span<const MemberInfo> members,
                 const std::type_info &std_type)
      : name_(name), size_(size), traits_(trait_flags), members_(members),
        std_type_(std_type)
  ...
  /**
   * @returns Constant @ref msce::MemberInfo reference representation of member
   * with given name.
   * @throws std::runtime_error if member with given name does not exist in this
   * type.
   */
   ...
   * @throws std::runtime_error if member with given name does not exist in
   * given object.
   */
  template <typename TValue, ClassWithReflection TObject>
  ...
  /**
   * @brief Sets the value of member with given name in given object to provided
   * one.
   * @throws std::runtime_error if member with given name does not exist in
   * given object.
   */
  template <typename TValue, ClassWithReflection TObject>
  void set_member_value(const TObject &object, const char *member_name,
                        TValue value) const;

  template <typename TValue>
  void set_member_value(const void *object, const char *member_name,
                        const Type &o_type, TValue value) const;

  template <typename TValue>
  TValue get_value(
      std::remove_pointer_t<std::remove_reference_t<TValue>> &target) const;

  template <typename TValue> TValue get_value(const void *target) const;

  template <typename TValue>
  void set_value(std::remove_pointer_t<std::remove_reference_t<TValue>> &target,
                 TValue value) const;

  template <typename TValue> void set_value(void *target, TValue value) const;
  ...
  /* I am absolutely NOT documenting 24 self-explainatory methods. Go back to
   * pre-school if you can't figure it out.*/
...
inline constexpr msce::MemberInfo::MemberInfo(const char *name, uint32_t offset,
                                              const Type &type)
    : name_(name), offset_(offset), type_info_(type)
{
}
...
{
  return this->get_value<TValue>(reinterpret_cast<const void *>(&object),
                                 typeof(TObject));
}
template <typename TValue>
inline TValue MemberInfo::get_value(const void *object,
                                    const Type &o_type) const
{
  if (!object)
  {
  ...
  {
    logger.log_error(
        "Tried to get value of '{}' in '{}', but it has no such member.",
        this->get_name(), o_type.get_name());
    throw std::runtime_error("Invalid reflection operation.");
  }
  ...
  if constexpr (typeof(TValue).is_integer())
  {
    if (this->get_type().is_enum())
    {
      ...
      logger.log_warning("Invalid integer size '{}'. Returning default...",
                         this->get_type().get_size());
      return TValue();
    }
  }
...
{
  this->set_value(reinterpret_cast<const void *>(&object), value,
                  typeof(TObject));

  // if (!TObject::get_type_info().has_member(*this))
  //     throw std::runtime_error("Tried to set value of member '" +
  //     this->get_name_str() + "' in '" +
  //     TObject::get_type_info().get_name_str() + "', but said object doesn't
  //     have said member.");

  // auto m = this->get_ptr<TValue>(object);
  // if (!m)
  // {
  //     logger.log_error("Tried to set value of member '{}' in object {}, but
  //     said object has no such member", this->get_name_str(),
  //     TObject::get_type_info().get_name_str()); return;
  // }
  // *m = value;
}
template <typename TValue>
inline void MemberInfo::set_value(const void *object, TValue value,
                                  const Type &o_type) const
{
  if (!o_type.has_member(*this))
    throw std::runtime_error(
        std::format("Tried to set value of member '{}' in '{}', but said "
                    "object doesn't have said member.",
                    this->get_name(), o_type.get_name()));
  ...
  if (!m)
  {
    logger.log_error("Tried to set value of member '{}' in object {}, but said "
...
inline TValue *MemberInfo::get_ptr(const TObject &object) const
{
  ...
  if constexpr (std::is_integral_v<TValue> || std::is_enum_v<TValue>)
    if (this->type_info_.is_enum() &&
        this->type_info_.get_size() == sizeof(TValue))
    {
  ...
  if (this->type_info_.get_std_type() != typeid(TValue))
  {
    logger.log_error("Tried to set value of member '{}' in object {}, but said "
                     "object has no such member",
                     this->get_name_str(),
                     TObject::get_type_info().get_name_str());
    return nullptr;
  }
...
inline TValue *MemberInfo::get_ptr(const void *object, const Type &o_type) const
{
  ...
  if (!o_type.has_member(*this))
  {
    logger.log_error("Tried to get pointer-to member '{}' in object '{}', but "
                     "no such member in this object exists.",
                     o_type.get_name(), this->get_name());
    return nullptr;
  }
...
inline void *MemberInfo::get_ptr(const TObject &object) const
{
  ...
  if (this->offset_ > TObject::get_type_info().get_size()) return nullptr;
...
template <typename TValue, ClassWithReflection TObject>
inline TValue Type::get_member_value(const TObject &object,
...
template <typename TValue>
inline TValue Type::get_member_value(const void *object,
                                     const char *member_name) const
{
  const auto &m = this->get_member_named(member_name);
...
template <typename TValue, ClassWithReflection TObject>
inline void Type::set_member_value(const TObject &object,
                                   const char *member_name, TValue value) const
{
  this->set_member_value<TValue>(reinterpret_cast<const void *>(&object),
                                 member_name, *this, value);
}

template <typename TValue>
inline void Type::set_member_value(const void *object, const char *member_name,
                                   const Type &o_type, TValue value) const
{
  const auto &m = this->get_member_named(member_name);
    ...
    std::remove_pointer_t<std::remove_reference_t<TValue>> &target) const
{
  ...
  else
  {
    ...
    {
      logger.log_error("Tried to get copy-of-value of type '{}', but it is not "
                       "copy-constructible",
                       this->get_name_str());
    }
...
inline TValue Type::get_value(const void *target) const
{
  ...
  if (t != *this)
  {
    logger.log_error("Tried to get value of type '{}' from object of type '{}'",
                     t.get_name(), this->get_name());
  ...
  if (!target)
  {
    ...
    logger.log_error(
        "Tried to get value of nullptr object of expected type '{}'",
        this->get_name());
    throw std::runtime_error(
        "Failed to get value of null object trough reflection");
  }
  ...
  else
  {
    if constexpr (get_reflection_of_type<TValue>().is_reference() ||
                  get_reflection_of_type<TValue>().is_copy_constructible())
    {
      return *v_ptr;
    }
    ...
    {
      logger.log_error(
          "Tried to return copy of non-copy-constructible type '{}'",
          this->get_name());
...
template <typename TValue>
inline void
Type::set_value(std::remove_pointer_t<std::remove_reference_t<TValue>> &target,
                TValue value) const
{
  using RawTValue = std::remove_pointer_t<std::remove_reference_t<TValue>>;
  if (get_reflection_of_type<TValue>() != *this) return;

  void *v_ptr = reinterpret_cast<void *>(&target);

  if constexpr (get_reflection_of_type<TValue>().is_reference())
  {
    logger.log_error("Failed to set value of reference type '{}'. You can not "
                     "reassign references after initialization.",
                     this->get_name_str());
    return;
  }
...
{
  using RawType =
      std::remove_cv_t<std::remove_reference_t<std::remove_pointer_t<TValue>>>;
  if constexpr (get_reflection_of_type<TValue>().is_reference())
  {
    logger.log_error(
        "Tried to set reference value post-initializarion for type '{}'",
        this->get_name());
    return;
  }
  ...
  {
    logger.log_error(
        "Tried to set value of constant post-initializarion for type '{}'",
        this->get_name());
    return;
  }

  if (typeid(value) != this->get_std_type()) return;
  ...
  else
  {
    ...
    logger.log_error(
        "Tried to set value of non-copy-constructible type '{}' by copy.",
        this->get_name());
  }
}
...
{
  return (this->traits_ & (uint16_t)TypeTraitFlags::INTEGRAL) ==
         (uint16_t)TypeTraitFlags::INTEGRAL;
}

inline constexpr bool msce::Type::is_floating_point() const
{
  return (this->traits_ & (uint16_t)TypeTraitFlags::FLOATING_POINT) ==
         (uint16_t)TypeTraitFlags::FLOATING_POINT;
}

inline constexpr bool msce::Type::is_unsigned() const
{
  return (this->traits_ & (uint16_t)TypeTraitFlags::UNSIGNED) ==
         (uint16_t)TypeTraitFlags::UNSIGNED;
}
...
{
  return (this->traits_ & (uint16_t)TypeTraitFlags::MEMBER_OBJECT_POINTER) ==
         (uint16_t)TypeTraitFlags::MEMBER_OBJECT_POINTER;
}
...
{
  return (this->traits_ & (uint16_t)TypeTraitFlags::LVALUE) ==
         (uint16_t)TypeTraitFlags::LVALUE;
}

inline constexpr bool msce::Type::is_rvalue_reference() const
{
  return (this->traits_ & (uint16_t)TypeTraitFlags::RVALUE) ==
         (uint16_t)TypeTraitFlags::RVALUE;
}
...
{
  return (this->traits_ & (uint16_t)TypeTraitFlags::BOUNDED) ==
         (uint16_t)TypeTraitFlags::BOUNDED;
}
...
#define MSCE_FUNDAMENTAL_HELPER(TypeName, UID)                                 \
  ...
  {                                                                            \
  template <> struct TypeRegistration<TypeName>                                \
  {                                                                            \
    inline static constexpr msce::Type type =                                  \
        msce::Type(#TypeName, sizeof(TypeName),                                \
    ...
    {                                                                          \
      ...
      {                                                                        \
        ...
        ::msce::internal::get_g_reflection_types_registry().register_entry(    \
            #TypeName, ::std::cref(TypeRegistration<TypeName>::type));         \
        logger.log_info("Successfully reflected type '{}'",                    \
                        TypeRegistration<TypeName>::type.get_name_str());      \
...
#define MSCE_REFLECT_CLASS_MEMBER_IMPL(ClassName, MemberName)                  \
  ::msce::MemberInfo(                                                          \
      #MemberName, __builtin_offsetof(ClassName, MemberName),                  \
      ::msce::get_reflection_of_type<decltype(ClassName::MemberName)>())
...
#define MSCE_REFLECT_CLASS_HELPER(ClassName, UID, ...)                         \
  namespace msce::internal                                                     \
  {                                                                            \
  template <> struct TypeRegistration<ClassName>                               \
  {                                                                            \
    ...
    inline static constexpr msce::Type type =                                  \
        msce::Type(#ClassName, sizeof(ClassName),                              \
                   ...
                   std::span(members), typeid(ClassName));                     \
    inline static constexpr const msce::Type &get_type() { return type; }      \
    static void register_self()                                                \
    {                                                                          \
      static const ::msce::Type &r = []()                                      \
      {                                                                        \
        Logger logger("StaticTypeRegistration");                               \
        ::msce::internal::get_g_reflection_types_registry().register_entry(    \
            #ClassName, ::std::cref(TypeRegistration<ClassName>::type));       \
        logger.log_info("Successfully reflected type '{}'",                    \
                        TypeRegistration<ClassName>::type.get_name_str());     \
...
#define MSCE_REFLECT_CLASS(ClassName, ...)                                     \
  MSCE_REFLECT_CLASS_HELPER(ClassName, __COUNTER__, __VA_ARGS__)
