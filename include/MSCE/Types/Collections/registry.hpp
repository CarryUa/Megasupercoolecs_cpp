...
namespace msce
{
template <class TIdentifyer, class TRegistered> class Registry
{
protected:
...
public:
  ...
  TRegistered &get_entry(TIdentifyer id)
  {
    static Logger logger("Registry");
    {
      if (!registry_.contains(id))
      {
        ...
        logger.log_error(
            "Tried to get registration with id that wasn't registered. '{}'",
            info.filename);
  ...
  inline bool is_registered(TIdentifyer id) { return registry_.contains(id); }

  /// @return A copy of registry map.
  std::unordered_map<TIdentifyer, TRegistered> enumerate_registry()
  {
